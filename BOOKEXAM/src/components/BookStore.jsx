...
function BookStore() {
  ...
  useEffect(() => {
    fetch('https://api.nytimes.com/svc/books/v3/lists/current/hardcover-fiction.json?api-key=r7R55yvDhh24T46q5Rvy0xdOfAAaL0Aa')
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <div className='bg-[#47302f] h-screen w-full'>
      <div className='flex justify-between w-full bg-[#47302f] '>
        <div className='flex'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mt-6 lg:ml-14 gap-5 py-4 bg-[#47302f] w-full text-[#6d6669]'>
        {books.map((book) => (
          <div key={book.primary_isbn13} className="card bg-[#352328] shadow-xl p-4">
            <Link to={`/book/${book.primary_isbn13}`}>
              <img src={book.book_image} alt={book.title} className="w-full h-auto" />
              <h2 className="text-lg font-bold">{book.title}</h2>
              <p>By {book.author}</p>
            </Link>
            <div className="mt-4">
              <button onClick={() => addToFavorites(book)} className="btn bg-[#efe3c8] mr-2">
                Add to Favorites
              </button>
              <button onClick={() => markAsRead(book)} className="btn bg-[#efe3c8]">
                Mark as Read
              </button>
            </div>
          </div>
